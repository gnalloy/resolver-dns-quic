# ADR-001：保持 resolver-dns-quic 为独立 Gnalloy 模块
...
`resolver-dns-quic` 作为 `gnalloy.org/resolver-dns-quic` 发布，并拥有该职责：Gnalloy resolver 的 DNS-over-QUIC exchanger，基于 RFC 9250 ALPN doq 与 QUIC stream。
